...
| | |
|---|---|
| Complete | `de`, `es`, `fr`, `nl`, `ru`, `hi` |
| Partly done, 24% to 35% | `it` `pt` `zh` `zh-TW` `id` `ja` `ko` |
...
strings. A run of ten agents stopped mid-catalog on an account spend limit, which is why seven
