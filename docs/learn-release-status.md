...
Last updated: 2026-09-11 (false teaching in three Learn lessons rewritten)
...
- [x] 6. Delete the 12 parked topic strings from `values/strings.xml` **and all 44
      ...
      and each `_subtitle`. That is 540 lines. (Lesson prose lived in Kotlin at the time. It
      and the figure captions moved into `strings.xml` on 2026-08-28, so unparking a topic now
      means restoring its content keys too; see the session log.)
...
| Date | What happened |
|---|---|
| 2026-09-11 | Three false claims in lesson prose, answers untouched: ruler s2 said a non-zero start makes the reading *short* (it is long); negatives s2 said a negative is not a quantity; the quadratics unit summary said a squared term gives the curve two roots. Also hedged parabola s4 (roots defined as a pair) and solving s1 (zero-product property "solves a quadratic"). English plus the 12 locales that already had the keys. |
| 2026-08-28 | Lesson content moved to `strings.xml`. Setting the app to German showed the problem plainly: a German frame around English lessons, because the catalog was authored in Kotlin on purpose and never translated. 2,564 strings moved out of `ArithmeticContent`, `GeometryContent`, `ShapeGuide` and `RulesGuide`; every displayed field on the model is now a `StringResource`, keyed by where it sits in the catalog (`learn_<lesson>_s3_body`, `learn_unit_<unit>_q4_prompt`) so a step keeps its key when it moves. `Numeric.answer` stayed a literal - it is compared against what the learner types, not shown - as did ids and slugs. The content-QA tests that read the text (formulas that state their answer, figures that draw it, explanations tinted for a figure the review screen never shows, the fraction colour code) moved to `desktopTest/LearnContentTextTest` where a resource environment exists; `LearnCatalogTest` keeps everything structural. `checkLocalizations` reports the 2,564 as pending rather than failing, so the UI-chrome guarantee it exists for is not buried under them. Notation stayed behind: a review pointed out that `<string name="..._q3_o1">-11</string>` is not language, so options that are numbers, formulas, and worked problems and results are a `CatalogText.Value` written in place next to the figure they have to agree with - `mathOptions("-11", "-5", "5", "11")` - and only the 1,543 runs that are actually words reached `strings.xml`. Explanations keep their numbers inline rather than becoming placeholder templates, which would have made three hundred sentences harder to author without removing the risk; instead `check_localizations.py` now fails any catalog translation whose numbers differ from the English. Three more passes took out everything mechanical, each one found by review rather than by me, which is why the last of them ended in a check instead of another sweep: the ten polygon facts and the options naming an amount became `<plurals>` (`CatalogText.Counted`); the equations still sitting in `explanation`, `prompt`, `question` and worked `lines` moved inline as `math(...)`; the 35 options that were a sentence with a number in it (`enlargement by 2`) became 13 templates and 4 plurals filled by `CatalogText.Formatted`; the 24 sentences the catalog asks twice with different numbers became 24 templates; and 193 keys holding a text that already existed elsewhere (`Sphere` was stored ten times) were folded onto 71 shared keys. `check_localizations.py` now fails on any of the three shapes coming back - a numbered option, a repeated sentence, a repeated text - so this is guarded rather than reviewed. 1,242 keys and 9 plurals remain, every one a distinct sentence. The numbers still inside those sentences stay there on purpose: 312 of them repeat their own step's arithmetic and would go stale if it changed, but a number in a lesson sentence is either the sum or a fact being taught, nothing in the text separates them, and a rule that guessed wrong would let an edit silently break the maths. A pass that templated the ones a rule could identify was written, reviewed and reverted for exactly that reason - it half-decoupled sentences and left them looking arbitrary. `./gradlew learnNumberCoupling` reports the tie instead, and the localization check fails for any locale left behind. Verified by re-rendering all 1,568 frames: English output is unchanged. Translating them is a separate session. |
| 2026-08-28 | Localization gaps closed. The four shape/rules guide strings added with the guides had never reached a locale file, and `learn_topic_intro` still said "work down the list" in all 51 after the grid rework, so both were translated afresh. Bigger find: the figures captioned themselves in English from the UI layer - solid names and their face counts, `4 sides, 4 corners`, `3 lines of symmetry`, `area = 12 sq cm`, `4 rows`, `7 in each`, `gradient`, `mean`, `25% of 60` - about 32 captions that the content/chrome split says belong in `strings.xml` but nothing had ever moved there. They now resolve through the new `ui/components/learn/LearnVisualStrings.kt`, looked up in `LearnVisualCanvas` because a `DrawScope` cannot call `stringResource`, and filled in at draw time by `fillIn`. Five of them are `<plurals>` - the first in the project - because sides, corners, rows and symmetry lines all need agreement outside English; `scripts/check_localizations.py` learned to check plural keys, and every locale was verified against Compose's own CLDR rule data for every count a figure can print. Also fixed: `BullsAndCowsDemo` built its tutorial caption in English while the localized `bulls_and_cows_demo_step2/step3` were used only to reserve height, and `PrismClearLevelCard` hardcoded "Level N" instead of `level_label`. A second sweep with no minimum word length then found `" sd"` on the bell curve, which a three-letter filter had walked past, and `MentalCalculationGame`'s "Remember 7" hint, the one hint in the app that says anything: it now goes through a `hintMessage()` channel mirroring `solutionMessage()`. All 51 locales complete. |
